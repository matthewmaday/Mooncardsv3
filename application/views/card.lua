...
function LoadCard:new(params)
	...
	local function onOrientationChange( event )
		...
		screen:tweenObject(screen, screen.x, (display.contentWidth-screen.myWidth)*.5, screen.y, 0, 1, 1)
		screen:alignContent()
	...
	function screen:alignContent()
		...
		if system.orientation == "portrait" or system.orientation == "portraitUpsideDown" then
			...
			screen:tweenObject(screen, screen.x,self.centerX, screen.y, self.centerY-20,.5, 1)

			-- banner
			screen:tweenObject(pBanner, pBanner.x,0, -220, -150,1, 1)
			
			-- banner text
			screen:tweenObject(pBannerTxt,pBannerTxt.x,0, -225, -155,1, 1)

		else

			-- card
			pCard.xScale,pCard.yScale = .75,.75
			screen:tweenObject(screen, screen.x,self.centerY*.5, screen.y, self.centerX,.5, 1)
